 ...
 * @implements
 *   - kernel::sr::PusherGCAContext
 *   - kernel::sr::PusherSynchrotronDragContext
 *   - kernel::sr::PusherComptonDragContext
 *   - kernel::sr::PusherAtmosphereContext
 *   - kernel::sr::PusherContext
  ...
  namespace sr {
    ...
    struct PusherSynchrotronDragContext {
      ...
      PusherSynchrotronDragContext(real_t dt, real_t omegaB0, real_t gamma_rad, float mass)
        : coeff { static_cast<real_t>(0.1) * dt * omegaB0 /
                  (SQR(gamma_rad * mass)) } {}
    };
    ...
    struct PusherBoundaries {
      ...
      PusherBoundaries(const boundaries_t<PrtlBC>& boundaries) {
        ...
        if constexpr (D == Dim::_3D) {
          raise::ErrorIf(boundaries.size() < 3,
                         "boundaries defined incorrectly",
                         HERE);
